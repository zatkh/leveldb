...
// defult)
static inline int alloc_label(int type, int group_mode)
{
    int rv = syscall(__NR_alloc_label, type, group_mode);
    return rv ? rv : -errno;
}

// set a label for current task
static inline int set_task_label(unsigned long label, int op, int label_type,
                                 struct label_struct *user_label)
{
    int rv = syscall(__NR_set_task_label, label, op, label_type, user_label);
    return rv ? -errno : 0;
}
...
static inline int sys_create_labeled_file(const char *pathname, mode_t mode,
...
int flags = (CLONE_THREAD | CLONE_SIGHAND | CLONE_VM);
 ...
 /* register uint32_t result __asm__("r0");
  ...
  register uint32_t a5 __asm__("r3") = 0; 
...
int difc_create_label(int type, int group_mode)
{
    if (type == 0)
        return (int)alloc_label(PLUS_CAPABILITY, group_mode);
    else if (type == 1)
        return (int)alloc_label(MINUS_CAPABILITY, group_mode);

    return (int)alloc_label(PLUS_CAPABILITY | MINUS_CAPABILITY,
                            group_mode); // default is to give both t+ and t- capabilities
}
...
int difc_replace_labels(long secrecySet[], int sec_len, long integritySet[], int int_len)
{
    ...
    for (i = 0; i < sec_len; i++) {
        ...
        printf("[difc_replace_labels] cur_label.sList[%d]: %ld \n", (i + 1),
               cur_label.sList[i + 1]);
    }
    ...
    for (i = 0; i < int_len; i++) {
        ...
        printf("[difc_replace_labels] cur_label.iList[%d]: %ld \n", (i + 1),
               cur_label.iList[i + 1]);
    }
    return set_task_label(0, REPLACE_LABEL, 0, &cur_label);
}

int difc_add_label(unsigned long label, int label_type)
{

    return set_task_label(label, ADD_LABEL, label_type, NULL);
}

int difc_remove_label(unsigned long label, int label_type)
{

    return set_task_label(label, REMOVE_LABEL, label_type, NULL);
}

int create_labeled_dir(char *pathname, int mode, long secrecySet[], int sec_len,
                       long integritySet[], int int_len)
{
    int i;
...
int create_labeled_file(char *pathname, int mode, long secrecySet[], int sec_len,
                        long integritySet[], int int_len)
{
    int i;
    ...
    return sys_create_labeled_file(pathname, mode, &flabel);
}
